...
import { invalidateOwnerCache } from '../../../hooks/useOwnerList';
...
export default function OwnerFormScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error || !data) {
        setFetchError(error?.message || 'Owner not found.');
      } else {
        setOwner(data);
  ...
  async function handleCreate(values, { setSubmitting }) {
    ...
    try {
      ...
      setSaveError(err.message || 'Failed to create owner. Please try again.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (fetching) {
    return (
      <ScreenWrapper>
        <Header router={router} title="Edit Owner" />
    ...
    return (
      <ScreenWrapper>
        ...
        <View style={S.centerWrap}>
          ...
          <Text style={S.centerTitle}>Failed to load owner</Text>
  ...
  const joinDate = owner?.created_at
    ? new Date(owner.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    : null;

  return (
    <ScreenWrapper>
      <Header router={router} title={isEdit ? 'Edit Owner' : 'Add Owner'} />
        ...
        >
          ...
          {isEdit && owner && (
            <View style={S.identityCard}>
              <Avatar url={owner.avatar_url} name={owner.name || ''} size={hp(7)} />
              <View style={S.identityMeta}>
                <Text style={S.identityName} numberOfLines={1}>{owner.name || '—'}</Text>
                {joinDate && <Text style={S.identityJoin}>Joined {joinDate}</Text>}
                {owner.school_name ? (
                  <View style={S.metaRow}>
                    <Ionicons name="business-outline" size={12} color={Colors.muted} />
                    <Text style={S.identitySchool}>{owner.school_name}</Text>
                  </View>
                ) : (
                  <Text style={S.identityNoSchool}>No school assigned</Text>
                )}
              </View>
            </View>
          )}
          ...
          >
            {({ values, errors, touched, handleChange, handleBlur, handleSubmit, isSubmitting }) => (
              <View style={S.formGap}>
                      ...
                      <View style={S.statusLeft}>
                        ...
                        <View>
                          ...
                          <Text style={S.statusSub}>
                            {isActive ? 'Owner can log in and use the app.' : 'Owner is blocked from logging in.'}
                          </Text>
                        </View>
                      </View>
                      <Switch
                        ...
                        onValueChange={(val) => {
                          if (!val) {
                            Alert.alert(
                              'Deactivate Owner',
                              'This will block the owner from logging in. Continue?',
                              [
                ...
                <Button
                  title={isEdit ? 'Save Changes' : 'Add Owner'}
